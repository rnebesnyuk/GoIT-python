...
def main():
    while True:
        try:
            ...
            user_command = get_close_matches(parced_msg[0], action_commands+exit_commands)
            if len(user_command) == 0:
                    print("Assistant does not recognize your command. Please input your request.")
            else:
                cmd_words = user_command[0].split()
                user_cmd_len = len(cmd_words)
                if user_command[0] in exit_commands:
                    print(good_bye_handler())
                    quit()
                elif user_command[0] in action_commands:
                    if user_cmd_len == 1:
                        func_arg = parced_msg[1:]
                        if len(func_arg) == 0:
                            print(commands_func[user_command[0]]())
                        else:
                            func_arg = ' '.join(func_arg)
                            print(commands_func[user_command[0]](func_arg))
                    elif user_cmd_len == 2:
